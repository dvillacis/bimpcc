...
fig,ax = plt.subplots(2,len(args.results_dirs)+2,figsize=(12,4))
...
for i,result_dir in enumerate(args.results_dirs):
    ...
    patch_size = int(np.sqrt(len(param)))
    
    ax[0,i+2].imshow(recons,cmap='gray')
    ax[0,i+2].set_title(f'{patch_size}x{patch_size}')
    ...
    Q = DiagonalPatchOperator((u,u),(patch_size,patch_size))
    p = Q*param
    v = int(np.sqrt(len(p)))
    par = ax[1,i+2].imshow(p.reshape((v,v)),cmap='gray')
